...
function autocomplete(input, latInput, lngInput) {
    if (!input) return; // Skip the function from running if there's no input
...
function typeAhead(search) {
    ...
    searchInput.on('input', function () {
        ...
        searchResults.style.display = 'block', _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
            if (res.data.length) {
